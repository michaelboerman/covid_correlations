...
Pull out just one date and plot:
```{r}
last_date_avail <- max(county_level_data$date) - 1
rand_date       <- sample(county_level_data$date, size = 1)
...
corrs <- cor.test(covid_one_date$cases, covid_one_date$pop_density)

covid_one_date %>%
  drop_na(cases, pop_density) %>%
  ggplot(aes(y = cases, x = pop_density)) +
  geom_point() +
  scale_y_log10() +
  scale_x_log10() +
  ...
  ggsave(here::here("plots/us_one_date_cases.jpg"), width = 12, height = 6)
```
