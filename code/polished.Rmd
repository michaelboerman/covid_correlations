...
# initialize a DF to put in each date's correlation
  cors_over_time <- data.frame(
    "date" = unique_dates,
    "n_countries" = rep(NA, length(unique_dates)),
    "death_cor" = rep(NA, length(unique_dates)),
    "case_cor" = rep(NA, length(unique_dates))
    )
  
# instead of a for loop, let's use R to its fullest!
# list of dates:
...
# for loop for calculating correlation per date
  for (i in 1:length(unique_dates)) {
    ...
    cors_over_time$n_countries[i] <- nrow(filtered_date_data)
...
# Set up with parameters so that I don't have to maintain a bunch of similar code.
# plot for correlation between pop density and deaths
...
for (i in length(series_covid)) {
cors_over_time %>% 
  ggplot(aes(x = date))+
  geom_line(aes(y = !! sym(series_covid[i]))) + 
  theme_minimal() +
  geom_hline(yintercept = 0)+
  geom_hline(yintercept = series_means[i],
  ...
  labs(title = paste0("Correlation between Population Density and COVID ", series_title[i]),
       subtitle = "Each correlation shown is the calculated using approximately 200 countries for that given date.",
  ...
  ggsave(filename = here(paste0("plots/cor_density_", series_title[i], ".jpg")))
}
```

Repeat, but for covid cases:
```{r}
# Now let's do away iwth icky for loop
pmap(list(
      series_covid <- c("death_cor", "case_cor"),
      series_title <- c("Deaths", "Cases"),
      series_means <- c(mean(cors_over_time$death_cor),
